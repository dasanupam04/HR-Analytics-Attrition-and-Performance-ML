...
if missing_models and not st.session_state.models_loaded:
    st.error("⚠️ **CRITICAL SYSTEM ERROR**")
    st.markdown(f"""
...
def render_landing_page():
    ...
    with col2:
        if st.button("⚡ INITIALIZE SYSTEM", key="enter_system", use_container_width=True, type="primary"):
...
with st.sidebar:
    ...
    with st.expander("⚙️ SYSTEM SETTINGS"):
        ...
        st.markdown("---")
        if st.button("↩️ Return to Landing", use_container_width=True):
            st.session_state.show_landing = True
            st.rerun()
    ...
    with status_container:
        ...
        logs = [
            f"<span style='color: {theme['accent']};'>●</span> System Online [{current_time}]",
            f"<span style='color: {theme['accent']};'>●</span> Models Loaded: 3/3",
            f"<span style='color: {theme['accent']};'>●</span> Predictions: {len(st.session_state.prediction_history)}",
            f"<span style='color: {theme['accent']};'>●</span> Quantum Entropy: 0.04",
            f"<span style='color: {theme['primary']};'>●</span> Status: <span class='status-badge status-success'>Optimal</span>"
        ]
...
if module == "FINANCIAL":
    ...
    if years_at_company > total_working_years:
        st.warning("⚠️ Org Tenure cannot exceed Total Working Years")
        valid = False
    if years_in_current_role > years_at_company:
        st.warning("⚠️ Role Specificity cannot exceed Org Tenure")
        valid = False
    ...
    if st.button("EXECUTE PROJECTION", disabled=not valid):
        with st.spinner("Decoding Neural Patterns..."):
            try:
                ...
                # Save to history
                save_prediction("FINANCIAL", input_df.to_dict('records')[0], f"₹{pred:,.0f}")
                
                st.markdown(f"""
                    <div class="metric-hero">
                        <p class="mono-label">Projected Monthly Value</p>
                        <h2 style="font-size: 6rem; font-weight: 800; margin:0;">₹{pred:,.0f}</h2>
...
# ==============================
    ...
    with c1:
        ...
        income = st.number_input(
            ...
            0, 250000, 15000,
            step=1000,
            help=get_help_text("Monthly income in ₹")
        )
        job_level = st.selectbox(
            "HIERARCHY LEVEL",
            [1, 2, 3, 4, 5],
    ...
    if st.button("DETERMINE ALIGNMENT"):
        with st.spinner("Mapping Talent Vector..."):
            try:
                ...
                st.markdown("<br>", unsafe_allow_html=True)
                with st.expander("🎯 ALTERNATIVE PLACEMENTS"):
...
if st.session_state.prediction_history:
    ...
    with st.expander("📜 VIEW COMPLETE HISTORY", expanded=False):
...
st.markdown(f"""
    <div style='text-align: center; padding: 2rem; color: #666;'>
        ...
        <p style='font-size: 0.7rem; margin-top: 1rem; opacity: 0.5;'>
            ...
            Status: <span style='color: {theme["accent"]};'>●</span> Operational
        </p>
    </div>
""", unsafe_allow_html=True)